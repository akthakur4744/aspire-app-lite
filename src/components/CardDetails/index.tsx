...
export default function CardDetailsSection(CardDetailsSectionProps: CardDetailsSectionProps) {
  ...
  return (
    <Container sx={{ background: "#FFFFFF", padding: "24px" }}>
      ...
      <Accordion sx={{  border: "1px solid #F0F0F0", borderRadius: "10px" }} defaultExpanded={true}>
        <AccordionSummary
          indicator={<img src={DownArrow} alt="down_arrow" />}
          sx={{ background: "#FAFCFF", height: "72px", borderRadius: "10px 10px 0 0" }}
        >
          <img src={CardSectionLogo} alt="trans_logo" /> Card Details
        </AccordionSummary>
        <AccordionDetails sx={{ border: "#F5F5F5" }}>
          {
            activeCard && (Object.entries(activeCard).map(([key, value]) => {
              return (
                <CardDetailComponent attributeName={key} attributeValue={value} />
              )
            } ))
          }
        </AccordionDetails>
      ...
      <Accordion sx={{ border: "1px solid #F5F5F5",  borderRadius: "10px" }} defaultExpanded={true}>
        <AccordionSummary
          indicator={<img src={DownArrow} alt="down_arrow" />}
          sx={{ background: "#FAFCFF", height: "72px", borderRadius: "10px 10px 0 0" }}
        >
